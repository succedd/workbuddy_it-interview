...
- **本地 git 分支现状（08-31 19:00 记录）**：本地仅有 `restore-features` 分支（tip `bab52f0`）。远端（`git ls-remote`）：`release` = `bab52f0`（与本地一致）、`main` = `0adecef`（与 release 已分叉，属常态：main 上有 Contents API 单文件提交）。**本次双推已完成**：① `git push origin restore-features:release` fast-forward 成功；② 用 Contents API 把 9 个文件（.gitignore / README.md / HANDOVER.md / css/style.css / index.html / js/app.js / js/guide.js / js/panorama.js / sw.js）逐个推到 main，全部 HTTP 200/201。**禁止 force push**
