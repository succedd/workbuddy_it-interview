...
- **最后更新**：2026-09-01 19:42
- **最新 commit（已双推 release+main，两分支 tip 同为 `0b77901`）**：**题库全景图点击无反应三连修（缓存版本 `20260901h`，部署于 2026-09-01 19:40 GMT+8）**——用户反馈「对应的题目点没反应」，用 Playwright+Chromium 真实点击实测复现并修复三个独立根因：① **`App` 加载时快照为 undefined**：panorama.js 先于 app.js 加载，旧代码 `const App = window.App` 固化成 undefined，所有节点点击与 tooltip「查看题目 →」的 `App.go()` 均抛 `TypeError: Cannot read properties of undefined (reading 'go')`（异常还中断 ECharts 事件循环）——改为运行时 `window.App.go` / `window.App.registerChart`（registerChart 恢复后图表重新随路由 dispose，不再泄漏）；② **岗位视图整棵树只剩中心点**：`buildPosTree()` / `renderPos()` / `summaryHtml()` 误用不存在的 `st.positions`（`positionsByStage()` 实际返回 `st.list`），已改 `st.list`；③ **结构节点点击展开/收起无效**：实测证实 ECharts tree 事件 `params.data` 是副本而非原引用，改它的 `collapsed` 写不回原树——`drawMindMap` 给每个节点发唯一 `_nid`，点击按 `_nid` 在原树定位真节点再切换折叠。回归 7/7 PASS（all 体系直达 / cat 叶子直达 / ai 题目直达+tooltip 链接 / pos 岗位直达 / 工具条展开-收起-复位），页面报错清零。**git 已恢复可用**：本机 PATH 仍无 git，但 WorkBuddy 便携版 `C:\Users\Life\.workbuddy\binaries\PortableGit\versions\1.2.0\cmd\git.exe` + 仓库 origin URL 内的 PAT + 仓库级代理 127.0.0.1:7897 均正常，本次全程本地 commit + fast-forward 双推（未 force）。因本地 restore-features 与线上存在 150+ 平行历史（历次会话 API 直推所致），推送路线为：基于 origin/main 开 `tmp-panorama-h` → 只带 h 修复 3 文件（panorama.js/index.html/sw.js，sitemap/HANDOVER 以线上为准防覆盖扩充记录）→ merge origin/release（`-X ours`，两分支实质仅换行差异）→ 双推；原 restore-features 已备份为本地分支 `backup-restore-features-0901`。
