...
- **最后更新**：2026-08-30 19:55
- **最新 commit**：本次提交（缓存版本 `20260829t`）——**柱状轨道塌缩修复**：.bar-track 的 flex:1 在自适应高度父容器中 flex-basis=0 导致高度塌成 0（这就是「柱子看不到」的真因），改为固定 40px + 虚线可见轨道；上一条（s）：**柱子零值可见**——**柱子零值可见**（0 也显示数字+小柱根，非今日柱色加深；无头测试确认周报运行正常）。上一条（r）：**热力图改周柱对比**；期间：用户浏览器吸收 76 道必读题后发布 340 题（双分支，闭环验证成功）——**热力图改周柱对比**（用户反馈柱状图更直观，8 根周柱带数字标注，纯 HTML/CSS）。上一条（q）：**周报图表可读性**——**周报图表可读性**：柱子标注当天题数、热力图加图例刻度与点击详情、标题大白话。上一条（p）：**学习周报三件套**——**学习周报三件套**：8 周日历热力图（ECharts calendar）+ 周目标 confetti 彩带（vendor/canvas-confetti，按需加载，每周期一次）+ 周报分享图（sharecard.js 新增 WeekCard，复用复制/保存/长按交互）；手册同步。上一条（o）：**电脑端分享修复**——**电脑端分享修复**：区分手机/电脑微信、新增「复制图片」按钮（剪贴板→微信 Ctrl+V）、系统分享失败自动降级保存；上一条（n）：**第一性原理必读题**——**第一性原理必读题**：19 域 ×4 题（诞生/本质/边界/演化）入库 265~340、分类页置顶必读机制（tags「第一性原理」识别）；题库 264→340 题，版本号随 editor 直写会回退为 1 属已知现象（同步按 publishedAt 判定）。上一条（m）：**在线手册补同步**——**在线手册补同步**：分享卡片（分层摘要排版）、独立分享页（单步直达 + 读完引导）、技术全景图（架构图/分支树双入口直达）三处说明与实际功能对齐；使用指南「文档最近更新」随每次功能更新必须同步（用户重申此纪律）。上一条（l）：**分支树节点点击直达分类**——**分支树节点点击直达分类**（末级节点导航、上层节点展开/收起；架构图方块此前已可点击）。上一条（k）：**人工架构图**——**人工架构图**：19 个重点技术域手绘分层架构图（data/tech-maps.json，173 节点 100% 可点击跳分类），分类页「架构图|分支树」双标签，叶子分类继承所属域全景；上一条（j）：**技术全景图自动树**（ECharts 按需加载、薄弱橙标、叶子显示同级位置）。上一条（i）：**分享卡片答案摘要重排**：旧版把答案整体去格式后硬折 7 行，编号/要点/分段全糊在一起（用户反馈：卡片排版乱到不想扫码）；现按编号/项目符号/空行切块、每块独立折行、块间留缝、行数用尽自动补省略号。同日：`9aa655a` 分享页读完引导条（滚动近文末滑入「连刷同类题」，可关闭）；`98e0795` 分享落地页取消 2.5s 自动跳转（单步直达，264 页重生成）；`c605519`（h）深链跳过开场动效；`31a61d3`（g）编辑端双推/Unicode 查重/吸收抑制/自愈；代码线 `aaf757b`/`73bd01c`
