...
- **本地 git 分支现状（08-31 21:30 记录）**：本环境 `git` 不可用，本地改动**未走本地 commit**，直接通过 Contents API（`tools/enrich_questions.py` 的 `github_put`）双推 `release`+`main`——本轮 5 个文件（css/style.css / index.html / js/panorama.js / sw.js / HANDOVER.md）全部 200 推送成功，远端 `release` 与 `main` 现均含 `v20260831c` 全景图轨道图。下次本地 clone/同步即与线上一致。**禁止 force push**
...
- **已上线功能**：题库浏览/搜索/刷题/收藏/错题本（间隔复习）/模拟面试（报告云端+历次趋势）/学习周报/每日打卡上云/PWA 离线（第三方库已本地化，真离线可用）/无障碍/题目分享卡片（canvas 图片 + 264 个内容化 SEO 落地页（单步直达 + 读完引导条）+ 微信长按适配/技术全景图（19 域人工架构图 + 自动分支树，薄弱标记）/第一性原理必读题（19 域置顶 4 题）/学习周报三件套（热力图+彩带+分享图））/sendBeacon 兜底同步/图片外置上传/**题库全景图（v20260901c，首页 4 个统计卡片钻取 → ECharts 思维导图：all/cat/pos 为技术/岗位演进的树状逻辑图，已下钻到具体技术（如 数据库→关系型数据库→MySQL）与子岗位、节点标签直接带题数、连线加粗按分组着色、径向/左右布局可切、逐级下钻折叠、大小∝题量、点击节点直达题单/岗位/题目、任一视图均可一键全屏；ai 视图为「题目来源」思维导图，可一路下钻到具体题）**
- **进行中/待办**：
