...
- **最后更新**：2026-09-05 20:55（**sw.js / index.html 缓存版本号统一升至 e**）
- **缓存版本号统一升级 `20260905d` → `20260905e`**——用户进一步澄清："页面样式不一样"指的是**题库全景（思维导图）页内**（`#/panorama`），不是首页 4 个统计卡。深挖：① 全栈字节比对（含 CRLF 归一化）确认 `index.html / css / sw.js / js/panorama.js / js/app.js / data/published.json` 与 `ced3cc6f`（事故修复基线）**完全相同**，无代码回退；② live `published.json` 含 481 题（id 1–481 完整），发布数据也对；③ 发现**长期遗留的视觉缺口**：`js/panorama.js` 使用 `pan-mm-* / pan-orbit-* / pan-fs-* / pan-legend-* / pan-tip-*` 等容器类，但 `css/style.css`（773 行，cr0 起就是这个状态）**对这些类没有任何样式规则**（仅 1 条 `.pan-loading`），所以轨道环不可见、工具条/全屏按钮无定位、legend 裸排——这从 `ced3cc6f` 一直如此；④ 真因最可能是用户浏览器 SW 缓存持有更旧版本的 `pan-*` 样式（早于 `v20260901a` 之前的轨道版），对比"现在的无样式"觉得差异明显。**应对**：把 `index.html` 全部 23 个 `?v=20260905c` 与 `sw.js?v=20260905c` 注册 URL **同步升到 `20260905e`**，配合 `sw.js` `VERSION = "20260905e"`——新旧 SW 文件 URL 不同，老 SW 不会复用旧逻辑缓存（`activate` 阶段删除 `iti-pwa-v20260905c/d`），**所有用户再开页面就强制拉新版**（无需手动硬刷新）。commit `86a26c2`（release + main 同步）。**强提示**：若仍看到差异，可能是浏览器还在跑旧 SW 进程——在 DevTools → Application → Service Workers 点 "Unregister" 即可，或关闭所有 `it-interview.is-a.dev` 标签页重开。**后续可选方案**（如仍嫌无样式）：为 `.pan-mm-* / .pan-fs-* / .pan-legend-*` 补齐 CSS 规则，锁死当前"无样式但可点击"基线，或对照一张"原先/现在"截图精修。
- **最后更新**：2026-09-05 18:15（**sw.js 缓存版本号升至 d**）
- **sw.js 缓存版本号更新 `20260905c` → `20260905d`**——用户反馈首页 4 个统计卡的"内部布局/样式"与原本印象中不同，可能性之一是 sw.js 缓存了旧版 CSS 导致浏览器看到旧样式（VERSION 不变则缓存不主动清）。升级 VERSION 后用户需**强制刷新一次**（Ctrl+Shift+R / Cmd+Shift+R）触发 activate 事件删除旧缓存 `iti-pwa-v20260905c`，下次启动即可拉到新版 CSS/JS。若强制刷新后仍有差异，则不是缓存问题——可能是用户最近（14:10 之后）改过样式没备份、被事故修复回退，需要用户提供「原先」截图才能精准恢复。commit `946d62c`（release + main 同步）。
