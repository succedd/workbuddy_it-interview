...
- **最后更新**：2026-08-30 14:20
- **最新 commit**：本次提交（缓存版本 `20260829i`）——**分享卡片答案摘要重排**：旧版把答案整体去格式后硬折 7 行，编号/要点/分段全糊在一起（用户反馈：卡片排版乱到不想扫码）；现按编号/项目符号/空行切块、每块独立折行、块间留缝、行数用尽自动补省略号。同日：`9aa655a` 分享页读完引导条（滚动近文末滑入「连刷同类题」，可关闭）；`98e0795` 分享落地页取消 2.5s 自动跳转（单步直达，264 页重生成）；`c605519`（h）深链跳过开场动效；`31a61d3`（g）编辑端双推/Unicode 查重/吸收抑制/自愈；代码线 `aaf757b`/`73bd01c`
