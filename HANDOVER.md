...
- **本地 git 分支现状（08-31 19:10 记录）**：本地仅有 `restore-features` 分支（tip `07ed849`）。远端（`git ls-remote`）：`release` = `0bc3827`（**正是本地的父提交，故本地 → release 可 fast-forward 直推**）、`main` = `bd82a347`（与 release 已分叉，需 Contents API 单文件推）。自动扩充流水线用 `--push`（Contents API）直接提交到线上 tip，本地 commit 不 push，故两边 hash 不同属常态。**本次改动涉及 js/css/html/sw/README，需：① `git push origin restore-features:release`（FF）；② 用 `tools/enrich_questions.py` 的 `github_put()` 把 7 个改动文件逐个推到 main；禁止 force push**
...
- **已上线功能**：题库浏览/搜索/刷题/收藏/错题本（间隔复习）/模拟面试（报告云端+历次趋势）/学习周报/每日打卡上云/PWA 离线（第三方库已本地化，真离线可用）/无障碍/题目分享卡片（canvas 图片 + 264 个内容化 SEO 落地页（单步直达 + 读完引导条）+ 微信长按适配/技术全景图（19 域人工架构图 + 自动分支树，薄弱标记）/第一性原理必读题（19 域置顶 4 题）/学习周报三件套（热力图+彩带+分享图））/sendBeacon 兜底同步/图片外置上传/**题库全景图（v20260831a，首页 4 个统计卡片钻取 → 旭日图/分类树/岗位树/AI 题清单）**
- **进行中/待办**：
