...
- **本地数据加密云备份**（v20260824a）：设置页可配置备份密码，把只存本机的数据（发布 Token、AI 配置与 Key、管理员密码、统计配置、收藏、浏览历史）以 AES-256-GCM 加密后备份到 `data/local-backup.json`（公开仓库上只有密文），随每次自动发布同步更新；清缓存/换设备后凭备份密码一键恢复。
