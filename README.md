...
- **题库全景图**（`v20260831a`）：首页 4 个统计卡片（技术分类 / 题目总数 / 覆盖岗位 / AI 生成题）均可点击进入 `#/panorama`，像思维导图一样展开全库结构，点击任意节点直达对应题目：
  ...
  - **技术分类**：279 个分类的可折叠树（一级默认展开），支持「只看有题目的分类」过滤，点击节点进入该分类（含子分类）题目列表。
...
### 2026-09-10 · 错题重练（艾宾浩斯复习）云同步补全（缓存版本 `20260910b`）
手机端看不到复习提醒横幅与错题重练内容。根因：复习数据为设备本地 IndexedDB 且原同步链路只传错题归属、不传复习进度（Worker `weak_bank` 表也无进度列）。本次：Worker 动态补 `box/due_at/marked/last_ok_at/updated_at` 五列，GET 返回全量进度，PUT 按 `updatedAt` 新者胜合并；前端上传/合并均携带并处理进度字段。端到端回环验证通过（含旧更新拒绝测试）。Worker 已部署（Version `647b8305`）。⚠️ 手机端实际生效仍需 API 可达（workers.dev 国内被墙，Netlify 桥待上线）。

### 2026-09-10 · 主题默认改浅色，修手机端整站变暗（缓存版本 `20260910a`）
用户反馈「电脑上视觉很好，手机上整个页面有点暗、刷题体验不好」。根因：首次访问默认主题为「跟随系统」，手机系统深色模式下整站自动切暗色。修复：首次访问默认主题改为**固定浅色**，不再跟随系统；顶栏切换按钮与设置页三档选择（浅色/深色/跟随系统）保留，已手动选过的用户不受影响。发版 v=20260910a（index.html 23 处 + sw.js VERSION）。

### 2026-09-09（晚） · 复习提醒视觉强化 + 岗位重复合并（缓存版本 `20260909g`，version 18）
复习提醒三处强化：首页到期横幅重做为**橙红渐变实底 + 52px 大数字圆徽章 + 呼吸光晕 + 白色实心 CTA**（旧版淡橙卡片+emoji 摆动被反馈不够醒目，且已定义的光晕动画此前未应用）；侧边栏到期徽章改**红底白字 + 光晕脉冲**；nav-pulse 小圆点同步改红。移动端 640px 断点适配。另：合并岗位表同名重复「公有云售后技术支持」5 条为 1 条（96/97/98/99→142，positions 142→138），22 道引用题去重收敛，悬空引用/pos 不一致/names 重复体检全绿。分享页 953 个重生成。线上验证 css/js 新类名与 published.json version 18 均已命中。

### 2026-09-09 · 题库扩充：网络抓取经典高频题 +58（895→953，version 17）
从权威源真实抓取经典高频面试题补强薄弱分类：**JavaGuide**（Java 基础/集合/并发/JVM/MySQL/Redis 六专题）、**CSDN 2026 后端面试汇总**（计算机网络 45 道、操作系统）、**面试鸭热门榜**、**牛客 2026 面经**（AI 工程化）。分布：MySQL×9（隔离级别/MVCC/索引失效/EXPLAIN/三大日志/深度分页/B+树/锁体系/主键设计）、Redis×8（为什么快/持久化/穿透击穿雪崩/分布式锁/淘汰策略/缓存一致性/跳表等）、网络×12（OSI/URL 全链路/ARP/DNS/握手挥手/拥塞控制/TCP vs UDP/粘包/epoll）、安全×2、OS×5（进程线程/IPC/死锁/虚拟内存/零拷贝）、Java×6（HashMap/ConcurrentHashMap/String/异常/接口抽象类/线程池）、JVM×6（内存区/GC Roots/四种引用/GC 算法/G1/CPU 飙高排查）、Spring×3、MQ×3、前端×8（事件循环/闭包/原型链/Vue3 响应式/React Hooks/HTTP 缓存）、AI×2（RAG/AI Agent）。全部落点题量薄弱分类，1 题与存量重复自动跳过，体检全绿（重复标题 0/答案过短 0/无岗位 0），分享页重生成 953 个 + sitemap 更新。同日还完成：学习打卡热力图月历式修复（视差）、发布守卫 + 编辑端自动同步升级（新版本提示胶囊缓存 20260909d）、223 题岗位全量关联（20260909e）、Netlify 国内直连桥方案（20260909f）。
