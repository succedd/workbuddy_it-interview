...
- **技术体系**：276 个分类的树状目录，支持按分类浏览题目。
- **岗位体系**：142 个岗位、146 条岗位技术栈，支持按岗位查看对应技能与题目。
...
### 2026-09-10 · 重复题合并第二轮：33 对（1081 → 1048 题）

承接上一轮的候选清单（`tools/dup-candidates-20260910.md`，57 对疑似重复），本轮**逐对人工核对**后合并 **30 组共 33 对**（同一道题被 3 个版本收录的，如 `Redis RDB/AOF`、`HashMap 原理`、`缓存穿透/击穿/雪崩`，一次并成一个）。题数 **1081 → 1048**，分类 276 / 岗位 142 / 岗位技能 146 全部不变。

**核对结论：57 对里只有 30 组是真重复**，其余 27 对是「同分类 + 用词相近但实质不同题」，**一律保留**，例如：
`Linux 进程管理命令` vs `Linux 网络命令`、`JavaScript 闭包` vs `Promise` vs `this` vs 事件循环、`Python 装饰器` vs `上下文管理器` vs 深拷贝浅拷贝、`MySQL 死锁` vs `主从延迟`、`Spring IoC/DI` vs `AOP` vs 事务 vs 异步、`K8s 核心组件` vs `Service 类型`、`Dockerfile 最佳实践` vs `Docker 网络模式`、`Kafka 消息不丢失` vs `顺序消费`、`HTTP 与 HTTPS` vs `HTTP/1.1·2·3 版本对比`、`Prompt Engineering 简答题` vs `Prompt Engineering 单选题`（题型不同）、`面试优缺点` vs `职业规划` 等。

**保留原则**：保留**答案更全**的那一版（多数为后补的 `web` 批次详版），被删版的 **views / favorites 累加到保留题**、`tags` / 岗位 / `relatedIds` 取**并集**，其他题指向被删题号的 `relatedIds` 一并改写（本轮累加 `views +4221`、`favorites +460`，修正关联 6 处）。

- 典型合并：`180 → 80`（Kafka 核心概念，142 字简版 → 359 字详版）、`179 → 78`（Spring Boot 全局异常，320 → 582 字）、`103 → 930`（ConcurrentHashMap，162 → 511 字）、`128 → 950`（React Hooks，157 → 611 字）、`823 → 208`（K8s CrashLoopBackOff，695 → 4611 字排查手册）
- 三版合一的：`43 + 139 → 906`（Redis RDB/AOF）、`4 + 100 → 929`（HashMap 原理）、`140 + 42 → 907`（缓存穿透/击穿/雪崩）
- **唯一用「拼装正文」的一对**：`909 → 142`（`Redis 过期删除和内存淘汰策略？`）。142 的题目覆盖「过期删除 + 内存淘汰」双主题但答案只有 148 字，909 只问淘汰策略却有 543 字详版——为不丢任何内容，给 `tools/merge-dup-questions.py` 增加 **`answerText`** 参数（写入合并后的正文），把两者拼成 756 字完整答案

**同步产出**：33 个被删题号的分享页改为**跳转页**（canonical + meta refresh + `noindex,follow`，指向保留题），`sitemap.xml` 同步移除这 33 条（**1086 → 1053 条**，其中分享页 1048 条）；`removedQuestions` 累计 37 条（含上一轮 4 条）。缓存版本 **`20260910e`**。

### 2026-09-10 · 重复题合并（1085 → 1081 题）+ 已删题号的本地清理

全库扫描发现重复题是系统性的：**字面重复只有 1 对**，另有约 **57 对「同题不同答案」**（早期 `seed`/`manual` 批次与后来的 `web` 批次各写一版）。本轮先合并**已确认的 4 对**：

- `853 → 536` 对称/非对称加密（保留版含 ECDHE/ChaCha20 与 TLS 组合动机）
- `753 → 983` Kotlin 相比 Java 的优势（保留版含平台类型坑、Google I/O 官宣）
- `755 → 986` Swift vs Objective-C（保留版含诞生动因、ARC、性能、互操作）
- `259 → 257` 移动端冷启动题（标题带「（副本）」的字面副本）

**处理原则**：浏览数/收藏数**累加**到保留题、`tags`/岗位/`relatedIds` 取**并集**（不丢热度与人工配置）。

**新增**：顶层字段 `removedQuestions`（{旧题号: 保留题号}）+ 前端 `C.applyRemovedQuestions`（清理本机残留并把收藏/浏览/错题记录重定向到保留题）+ 工具 `tools/merge-dup-questions.py`（`--scan`/`--dry`/`--apply`/`--push`/`--write-redirects`，幂等）。被删题号的分享页改为**跳转页**，sitemap 移除对应条目。

### 2026-09-10 · 重名分类合并（279 → 276 分类）+ 编辑端防回退
全库扫描出 **3 组同名重复分类**（建分类时没查重），合并规则为 **被岗位技能表引用 > 题多 > id 小**，题数不变（1085）：

- **Kotlin**：保留 Android 开发域 `id224`（5 题，被 Android开发工程师引用），移除「编程语言与编程基础」域的空壳 `id20`（0 题）
- **Swift**：保留 iOS 开发域 `id219`（5 题，被 iOS开发工程师引用），移除「编程语言与编程基础」域的空壳 `id21`（0 题）
- **网络安全基础**：保留「信息安全与网络安全」域 `id201`（被信息安全工程师引用），移除「计算机网络与协议」域 `id53`，其 6 道题改挂 id201（合并后 8 题）

本轮保留方恰好都是被岗位引用的那一个，因此 `positionSkills` 零改动。新增工具 **`tools/merge-dup-categories.py`**（`--dry / --apply / --push`），今后再出现重名分类可一条命令清理；脚本带完整性断言（未受影响的技能条目一条不动）。

**同时修掉根因**：编辑端本地库对分类只做「并集吸收」（只加不删），已清理的重复分类会在下次自动发布时被整包推回云端——而 `guardAgainstShrink` 只比对题数，拦不住分类结构回退。现在 `js/cloud.js` 的 `absorbRemote` 里新增**同名分类就地合并**（以云端为权威），编辑端一刷新即自动清掉本地重复项，不再回推。缓存版本 `20260910c`。
