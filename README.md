...
- **岗位学习路线图**（`v20260913a`）：把某个岗位已关联的题目**按技术分类聚合成 4–8 周的学习计划**，解决「142 个岗位、1000+ 道题，不知道该从哪开始、学到哪算完」的问题。入口 `#/roadmap`（侧栏「学习路线图」）：
  ...
  - **从路线图直接开练**：`#/practice?scope=roadmap&pos=<岗位 id>&week=<周次>`，题池由 `js/roadmap.js` 统一计算，与路线图页**完全同源**（不会出现「路线图说这周 33 题、练习里只有 12 题」的错位）；在练习页切难度/模式时会保留岗位与周次，并显示「来自 XX 学习路线图第 N 周」的上下文与返回入口。
