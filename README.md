...
> 按时间**逆序**记录（最新在最上方）；**同一天的变更合并在同一天条目下**，条目内按变更时间逆序。
...
**SW 导航请求绕过 HTTP 缓存，发版即生效（缓存版本 `20260827t`）**
...
**图标尺寸写进 SVG 属性，彻底根治巨型图标（缓存版本 `20260827s`）**
...
**修复结果页巨型图标导致内容不可见（缓存版本 `20260827r`）**
- **P0 修复**：`svg.ic` 原本无全局尺寸约束，结果页「面试完成」h2 内裸放的图标被浏览器按 viewBox 撑成 **917×917px 巨型图形**，把标题、统计卡片、账户提示条全部挤出视口——这是用户「看不到未登录引导」的真正根因（非缓存问题）。
- 新增全局兜底 `svg.ic { width:16px; height:16px }`；`.empty .em-ic svg.ic`（56px 大图标容器）与 `.tree-row .twist svg.ic`（12px）补恢复规则，已有 `.btn .ic` 等具体规则优先级更高不受影响。顺带修复搜索框图标同类隐患。
- 真实浏览器 E2E 验证：未登录走完 10 题模拟面试 → 结果页图标 16×16、「账户提示 · 去登录 →」蓝色提示条清晰可见可点。

**结果页提示升级为醒目提示条（缓存版本 `20260827q`）**
- 结果页「账户提示」由低调灰字（`class="muted"`）升级为带边框/底色的醒目提示条（加粗「账户提示 ·」标题），彻底解决「看不到」的问题；覆盖「未登录→去登录引导」与「已存云端→再面一次」两种状态。

**结果页未登录引导 + 历次趋势查看全部（缓存版本 `20260827p`）**
- feat: **结果页未登录引导**——模拟面试结果页「历次成绩趋势」此前在未登录时完全隐身（无任何提示）。现在未登录时显示「登录后可保存成绩到云端，并留存历次趋势对比 · 去登录 →」跳转 `#/account`，避免用户困惑。
- feat: **历次趋势「查看全部」**——面试记录超过 6 条时，默认仅展示最近 6 条并显示「查看全部」链接，点击展开更早记录（再次点击收起）。

**复习提醒增强 + 题目分享 + 面试报告云端趋势（缓存版本 `20260827o`）**
...
**复习闭环补全 + sendBeacon 兜底上传（缓存版本 `20260827m` / `20260827l`）**
