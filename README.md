...
### 2026-09-08 · 丢失题全量找回：677 → 874 题（git 历史考古 + 超集验证）

- **背景**：上轮发现 `q/` 有 71 个孤儿分享页（678-748）且部分旧页内容比当前 `published.json` 更丰富，遂做全量考古：遍历 git 历史全部 **140 个** `published.json` 版本（99 → 225 → … → 748 → 637 → 657 → 677），确认 2026-09-06 的 `147b792`（748→637）是最大一次塌陷。
- **恢复 197 题**：取「全历史按标题并集 + 最长答案」重建，当前缺失 197 题**全部找回**（ID 复用 678-748 的 71 道直接对齐旧分享页，其余 126 道按历史 commit 分组连续分配 749-874）。语义查重 0 撞车；分类/岗位引用全部有效；`relatedIds` 重映射 383 条全部命中、0 丢弃。
- **顺手修复 3 处历史遗留**：`#133` SQL优化答案被降质（152 → 3614 字符，从 748 版找回）；`#229/#233/#239` 三题 `categoryId=null`（分别归入 云原生与DevOps / 技术方案表达 / 常见HR面试题）。
- **安全性验证（决定性）**：把 748 个旧分享页正文与恢复后题库逐页比对——剥掉 markdown 渲染差异（加粗/代码块语言标记/列表编号）后 **741 页完全一致、7 页仅剩代码块语言标记丢失、0 页比题库更丰富**。结论：恢复后的题库是全部历史内容的**严格超集**，旧页无一字丢失。
- **分享页全量重生成**：874 页与题库一一对应（0 孤儿、0 缺页），sitemap 879 条。678-748 页仅时间戳刷新，482-517/617-637 页为「历史换题页」按当前题库重渲染（内容已在题库验证）。
- **数据规模**：874 题 / 279 分类 / 142 岗位。新增题难度分布：初级36 / 中级92 / 高级69；类型：简答150 / 场景18 / 故障排查15 / 编程6 / 开放讨论5 / 单选2 / 系统设计1。
...
- **改造 2：远程可配置入口**。新增同源 `api-endpoints.json`，页面每次加载拉取（缓存 6h），`sw.js` 对该文件特判 **network-first**（同源默认 cache-first 会把配置缓存死）。**eu.org 审核通过后只需把新地址写进这个 JSON，全量用户无需发版、无需清缓存即可切换后端入口**。
- **改造 3：报错文案说人话**。原来只说「请检查网络后点刷新重试」，现改为明确指出「国内网络访问 Cloudflare 的 workers.dev 域名常被拦截，请切换网络或到设置里点自动选择可用入口」。
- **改造 4：设置页新增「自动选择可用入口」按钮**（`#stats-autopick`）：逐个探测候选入口并列出 可用/不可达 + 耗时，自动切到第一个可用地址；访问统计模块 `Stats.cfApi()` 同步复用同一套入口解析。
