/* =========================================================================
 *  js/docs-data.js — 「学」版块：技术方向 × 分级 × 章节 的学习文档数据
 *
 *  设计原则（2026-09-14 定稿）：
...
`
  ...
  const DIRS = [
    OPS,
    skeleton("java", "Java 后端", "☕", "Java 基础、并发与 JVM、Spring 全家桶、MySQL/Redis 到分布式架构。", [
      ["初级", "能独立完成需求开发与调试。",
        ["Java 语法核心与集合框架", "异常与日志规范", "Maven/Gradle 与项目结构", "MySQL 基础与 SQL 编写", "Spring Boot 第一个服务", "Git 与团队协作流程"]],
      ["中级", "能定位线上问题、做方案设计。",
        ["JVM 内存模型与 GC 调优", "并发编程与线程池", "Spring 原理（IoC/AOP/事务）", "MySQL 索引与慢查询优化", "Redis 缓存设计与一致性", "消息队列与异步化", "分布式锁与幂等设计"]],
      ["高级", "能做架构决策与技术攻坚。",
        ["JVM 调优实战与故障排查", "MySQL 分库分表与高可用", "分布式事务与一致性", "高并发系统设计与压测", "微服务治理（注册/限流/熔断）", "性能工程与容量规划"]]
    ]),
    skeleton("frontend", "前端 Web", "🎨", "从 HTML/CSS/JS 基础到框架原理、工程化与性能优化。", [
      ["初级", "能还原设计稿、完成交互。",
        ["HTML 语义化与 CSS 布局", "JavaScript 核心语法", "DOM 与事件", "HTTP 与浏览器基础", "响应式与移动端适配"]],
      ["中级", "能独立负责模块、排查兼容与性能问题。",
        ["ES6+ 与异步编程", "框架原理（React/Vue）", "组件设计与状态管理", "构建工具与工程化", "TypeScript 实践"]],
      ["高级", "能做架构与技术选型。",
        ["浏览器渲染原理与性能优化", "前端监控与错误治理", "微前端与大型项目管理", "跨端方案与工程提效", "前端安全"]]
    ]),
    skeleton("dba", "数据库 / DBA", "🗄️", "关系型与 NoSQL 的原理、调优、高可用与数据治理。", [
      ["初级", "会写会查、懂基本运维。",
        ["SQL 基础与查询优化入门", "数据库对象设计与范式", "备份与恢复基本操作", "用户与权限管理"]],
      ["中级", "能独立优化与排障。",
        ["索引原理与执行计划", "锁与事务隔离级别", "慢查询治理", "主从复制与读写分离", "Redis 数据结构与持久化"]],
      ["高级", "能设计数据架构。",
        ["分库分表与中间件", "高可用与容灾方案", "容量规划与压测", "数据迁移与双写", "数据一致性校验"]]
    ]),
    ...
    skeleton("network", "网络与操作系统", "🌐", "计算机底层公共基础，所有方向的共同地基。", [
      ["初级", "建立完整的基础认知。",
        ["计算机网络体系与 TCP/IP", "HTTP 协议详解", "进程与线程", "内存管理基础"]],
      ["中级", "能解释现象、定位问题。",
        ["TCP 可靠传输与拥塞控制", "HTTPS/TLS 握手", "DNS 与 CDN", "IO 多路复用", "文件系统与磁盘 IO"]],
      ["高级", "能解决复杂底层问题。",
        ["内核网络栈与调优", "高并发网络模型", "容器网络原理", "系统调用与性能剖析"]]
    ]),
