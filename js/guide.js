...
async function pageHelp() {
  ...
  setMain(`
    ...
    `)}
    ${sec("roadmap", "🗺️ 学习路线图", `
      ${li("这是什么", "侧栏「学习路线图」把每个岗位的题库自动拆成一份 4～8 周的学习计划：先易后难排周次，每周给出要刷的题量、预计小时数和主攻技术点，不用自己规划「先看什么、后看什么」")}
      ${li("怎么用", "挑一个目标岗位（如「Java 后端开发工程师」）点进去，从第 1 周开始按顺序刷。每周都有「▶ 开始本周练习」按钮，直接进入该周的题目池，答完自动记进度")}
      ${li("进度怎么算", "在路线图里勾选、或在题目详情页点「已掌握」、或在练习中标记掌握，三处进度实时同步；刷新页面、换设备都不会丢")}
      ${li("每周自检", "每周卡片显示「已掌握 x/y」和进度条；全部掌握后周标题会打勾。一轮走完可以「重置进度」重新开始")}
      ${li("题量不足的岗位", "题库里题量过少（< 20 题）的岗位不会生成路线图，避免排不出有意义的周计划；这类岗位建议先按分类浏览")}
    `)}
    ${sec("docs", "📘 学习文档（「学」版块）", `
      ${li("这是什么", "按「技术方向 × 初级 / 中级 / 高级」组织的成套学习文档：一个方向一套文档，从初级第一章按顺序读到高级最后一章，就是一条完整的成长路线。与「刷题」的区别是——文档负责「系统地学」，题库负责「练和面」")}
      ${li("怎么进", "侧栏「📘 学习文档」→ 选方向（如「运维 / SRE」）→ 选级别 → 点章节开始读")}
      ...
      ${li("内容覆盖", "目前「运维 / SRE」方向初级 / 中级 / 高级共 9 篇已上线；其余方向（Java 后端、前端、数据库、云原生、网络与操作系统、安全）已规划好目录，标注「建设中」，内容陆续补齐")}
    `)}
    ${sec("daily", "📚 日常学习", `
    ...
    `)}
    <div class="muted" style="text-align:center;font-size:12px;margin-top:18px">文档最近更新：2026-09-14 · 更详细的开发文档见 GitHub 仓库 README</div>
  `);
